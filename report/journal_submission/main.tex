...
The middleware is a thin proxy between the analyst and a backend database (Figure~\ref{fig:arch}). Each query is parsed and validated, hashed to a (template, parameter) key, and checked against a two-level cache; a hit is returned for free (post-processing), a miss is charged $\eps_q$, noised, and cached. The model of Section~\ref{sec:model} is embedded in this pipeline rather than layered on top of it: every budget decision the middleware makes consults the same closed form---the per-query $\eps_q$ is sized from the forecast ($B/m$ or $B/\hat U$), a stale entry is re-noised only when the budget-gated re-release policy (Proposition~\ref{prop:barr}, below) permits it, and the cache key that determines what is charged at all is the equivalence class the forecast prices. Removing the model would not remove a feature; it would remove the component that decides how the budget is spent. Four core execution modes (exact, naive-DP, workload-aware-DP, temporal-DP) share the pipeline of Algorithm~\ref{alg:full}; two further modes extend it (predictive allocation, Appendix~\ref{app:allocator}; semantic caching, Appendix~\ref{app:semantic}). The pipeline couples cache validity to a temporal regime: it advances a logical clock, simulates per-step Bernoulli invalidations (each entry invalidated independently with probability $\lambda q_{\mathrm{inv}}$, a discrete approximation of a Poisson process), ages each entry against the staleness tolerance $\tau$, and only then returns the cached release or charges the ledger.
