...
\begin{abstract}
Differentially private (DP) SQL systems add calibrated noise to each aggregate query and charge it against a finite privacy budget; once the budget is exhausted, no further query can be answered. Existing systems account for each query in isolation, although analytical workloads repeat heavily and an exact repeat can be served from cache at no privacy cost by post-processing. A data custodian therefore cannot tell in advance whether a workload will fit its budget. We present a closed-form model that forecasts a workload's budget consumption before any query executes, from public repetition structure alone: the distribution over distinct queries, the workload length, and the budget. The model yields the expected spend, a hard completion guarantee under a safe per-query budget, and a per-release accuracy verdict. It extends to Markovian and regime-switching arrivals, and, because it is mechanism-agnostic, it transfers unchanged to multi-table joins and to equivalence-aware caching. We implement it in a DP-SQL middleware over DuckDB and PostgreSQL. Across a $4{,}155$-trial campaign the forecast matches observed consumption within $3\%$ in $21$ of $24$ configurations, and on $30$ workloads derived from real Amazon Redshift traces, forecasting from only the first half of each workload reduces the budget-prediction error by $45\%$.
\end{abstract}
...
While the \emph{magnitude} estimate above is not a bound, the re-release \emph{policy} that consumes it admits one. The planning estimate prices a schedule; the question a custodian actually faces is whether re-noising can overrun $B$. It cannot, if releases are budget-gated:
...
\noindent This is a worst-case \emph{guarantee}: it holds for every arrival process, unlike the magnitude estimate, and it makes the refresh rate a budgeting decision rather than a matter of hope. The guarantee also turns the refresh rate into a formal \emph{optimization} problem in the remaining budget \emph{and} the volatility $\lambda$. Consider minimising the time-average staleness, which is proportional to the refresh interval $\tau$, subject to the re-release budget constraint $\eps_q\,u_k\bigl(\lceil T/\tau\rceil+\lambda T q\bigr)\le B$ (the $\lambda Tq$ term is the volatility-driven spend of Proposition~\ref{prop:tempbound}). The objective is monotone increasing in $\tau$ while the feasible set is monotone in $\tau$, so the optimum sits on the budget boundary:
\begin{equation*}
...
Beyond pointwise agreement, the predicted invariances and limiting behaviour also hold empirically. The model's $\E[u_k]$ is structurally independent of $\eps_q$ (confirmed within $0.12$ units across $\eps_q\in\{0.1,0.5,1,2\}$) and of data scale: the same templates over a $10\times$ larger TPC-H database (60M rows) give identical empirical $\E[u_k]$ to within $0.034$ units, isolating workload structure from data scale. Pushing skew to $\alpha{=}10$ at $k{=}200$, predicted and empirical $\E[u_k]$ stay within $0.13$ units while $\E[u_k]$ itself varies by an order of magnitude, and $S(k)\to99.4\%$ recovers the $1-1/k$ corner numerically (Figure~\ref{fig:limits}).
...
The validation so far assumes arrivals that are independent and identically distributed (i.i.d.). To probe the forecast's robustness to this assumption, we extend the occupancy model to correlated and non-stationary arrival processes, beginning with a bursty ``sticky'' process.
...
\noindent This covers regime-switching workloads whose $\{p_i\}$ drifts with a latent state. On a two-regime chain, each regime concentrating on a different half of the templates, the closed form matches simulation to $\le0.03$, whereas a stationary i.i.d.\ forecast that ignores the regimes mis-predicts $u_k$ by up to $1.7$; the latent structure must therefore be modelled explicitly, which the proposition provides.
...
\section{Future Work}
\label{sec:future}
Several extensions follow directly from the model and its evaluation. (1) The Redbench validation uses trace-derived synthesis, so running the model on raw analyst query logs, and executing them through a DP engine end-to-end, would close the remaining external-validity gap. (2) Replacing basic sequential composition with R\'enyi DP~\cite{mironov2017renyi} or zCDP~\cite{bun2016concentrated} would predict the cumulative spend $\sum\eps_i$ directly rather than the distinct count $u_k$, with the closed-form structure mostly unchanged; this refinement composes with the caching, because tighter accounting lowers the per-release rate while caching lowers the number of paid releases, so the two act on orthogonal axes. (3) Section~\ref{sec:joins} executes single foreign-key joins live, under both the public $d_{\max}$ clamp and an instance-optimal elastic bound computed at runtime; extending the per-template sensitivity to multi-way and self-joins via residual sensitivity~\cite{dong2021residual} or R2T~\cite{dong2022r2t}, and switching the released mechanism to $(\eps,\delta)$, is the remaining step, and the forecast layer needs no change for any of these mechanisms. (4) The predicate-normalisation and Z3 SMT layers that make the forecast \emph{equivalence-aware} (Section~\ref{sec:model}, Appendix~\ref{app:semantic}) cover decidable predicate and integer-arithmetic equivalences; extending the SMT fragment to general \emph{join}-level equivalences remains open. (5) Re-noising stale cache entries when a budget surplus is available, together with a SUM$+$COUNT decomposition of AVG with per-group accounting, would tighten utility. (6) A stability-based threshold that suppresses low-count groups would lift the public-key-domain assumption at a small $\delta$ cost. (7) The staleness/update extension of Section~\ref{sec:system} now has a \emph{provable budget ceiling} under the budget-aware re-release policy (Proposition~\ref{prop:barr}) and a budget-feasible optimal refresh rate $\tau^\star$; what remains is the accuracy theory, namely predicting $\E[\eps_{\mathrm{temp}}]$ under a proper renewal arrival process, which deserves to be developed and validated as a study of its own. (8) The occupancy model now spans i.i.d., sticky, and general-Markov arrivals (Propositions~\ref{prop:sticky},~\ref{prop:markov}) and non-stationary regime-switching ones via a latent state (Proposition~\ref{prop:hmm}), so the arrival-process side of the model is closed; what remains for external validity is a second independent production trace beyond Redbench, and running raw production SQL end-to-end through a DP engine.

\section{Discussion and Conclusion}
\label{sec:discussion}

The experimental results support four findings. (1) Budget consumption is a function of $\E[u_k]$, not of the cache implementation: $\E[\eps_{\mathrm{wa}}]=\eps_q\E[u_k]$ holds within $<3\%$ in $21/24$ cells, exactly on the mixed workload, and is invariant to scale and $\eps_q$---so a deployment can estimate its privacy cost \emph{before} running a query. (2) The folklore $1-1/k$ rule is recovered as the high-skew corner of a single model. (3) The per-query calibration is clean: the single-query membership-inference attack (MIA) stays at or below the $e^\eps/(1+e^\eps)$ accuracy bound (within $1\%$ for $\eps\le1$) and reconstruction tracks $1.5/\eps$, while the workload-level shadow MIA was inconclusive at our sample size, a negative experimental result rather than a leakage claim. (4) The model also predicts where it fails. On \emph{unique} workloads (and uniform over a large or growing support) savings vanish and reconstruction succeeds, exactly as predicted; on a small finite pool, a uniform workload still saturates at $m\eps_q$ and continues to yield savings.
