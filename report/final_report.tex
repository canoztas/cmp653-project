...
\paragraph{Top-$k$ as post-processing.} The middleware also supports top-$k$ queries (\texttt{$\ldots$ GROUP BY $g$ ORDER BY agg [DESC$|$ASC] LIMIT $k$}), with one essential design point: the ordering and truncation act on the \emph{noised} histogram, never the true one. The engine fetches all $g$ groups, noises every count under the same parallel composition (cost $\eps_q$, \emph{not} $k\eps_q$), and only then sorts by the noisy value and keeps the top $k$; the raw \texttt{ORDER BY/LIMIT} is stripped before it reaches the database, because letting the DB apply it would select the reported groups on their \emph{true} counts---leaking which groups crossed the cutoff. Since the released set and its counts are a deterministic function of the $\eps_q$-DP noisy histogram, top-$k$ is post-processing and charges no extra budget (the report-noisy-max guarantee, here giving the whole top-$k$ slate and its noisy counts at the cost of one histogram). The price is that the selection is \emph{approximate}---groups whose true counts straddle the $k$-th boundary may swap---and it inherits the public-key-domain assumption above. The forecast and cache apply unchanged: a repeated top-$k$ query is one template, free on exact repeat and counted once in $\E[u_k]$.
...
We give two bounds. \emph{(Distribution-free.)} Changing a single $Q_t$ changes $u_k$ by at most $1$, so McDiarmid's bounded-differences inequality gives $\Prob[\,|u_k-\E[u_k]|\ge t\,]\le 2\exp(-2t^2/k)$. This is only a crude fallback: it scales with the workload length $k$ and becomes vacuous once $u_k$ saturates near its ceiling $m$. \emph{(Variance-aware.)} Write $u_k=\sum_i X_i$ with $X_i=\mathbf 1[\text{template }i\text{ appears}]$ and $q_i=1-(1-p_i)^k$. The occupancy indicators are negatively associated, so
\[
\Var[u_k] \;\le\; \sum_i q_i(1-q_i) \;=\; \sum_i \bigl(1-(1-p_i)^k\bigr)(1-p_i)^k \;=:\; V \;\le\; \tfrac{m}{4},
\]
and Chebyshev's inequality gives $\Prob[\,|u_k-\E[u_k]|\ge t\,]\le V/t^2$.
\end{prop}

\noindent Because $V=O(m)$ rather than $O(k)$, the variance-aware tail is the one to use in the saturated operating regime, and it is the honest bound on the budget-exhaustion event: for $c/\eps_q>\E[u_k]$, $\Prob[u_k\eps_q\ge c]\le V/(c/\eps_q-\E[u_k])^2$. (At $\alpha{=}1,m{=}10,k{=}100$, $\sqrt V=0.27$ matches the Monte-Carlo standard deviation of $u_k$ to two digits, whereas the McDiarmid sub-Gaussian scale $\sqrt{k}/2=5$ overstates the spread $\sim$18$\times$; we therefore report McDiarmid as a fallback only.) The estimator \texttt{occupancy\_variance} is implemented in \texttt{src/dpdb/model.py}.
...
\begin{prop}[Temporal budget upper bound]
\label{prop:temporal}
If the query-arrival process is independent of the data-update process, updates arrive as a Poisson($\lambda$) process and invalidate each cache entry independently with probability $q$, and a stale ($\mathrm{age}>\tau$) or invalidated entry is re-noised on its next query, then the expected re-noisings per template over horizon $T$ are at most
\[
N(T,\tau,\lambda,q) \;=\; \lceil T/\tau \rceil \;+\; T\lambda q,
\]
and the expected total workload-aware budget satisfies
\[
\E[\eps_{\mathrm{temp}}(T)] \;\le\; \eps_q \cdot \E[u_{\,k_{\mathrm{tot}}}] \cdot N(T,\tau,\lambda,q),
\]
where $k_{\mathrm{tot}}$ is the total number of queries issued in $T$.
\end{prop}
\begin{proof}
Per template, staleness re-noisings number at most $\lceil T/\tau\rceil$ (each resets the entry's age); each update-driven re-noising consumes a distinct invalidation event, so their count is at most the invalidations hitting the template, of expectation $\le T\lambda q$. Summing over the (independent) appeared templates gives $\E[u_{k_{\mathrm{tot}}}]\,N$; multiply by $\eps_q$. The bound holds in $12/12$ simulated cells---strict in the dynamic regime, tight (equality) in the static one (\texttt{experiments/temporal\_bound.py}). \end{proof}
...
\paragraph{A provable budget ceiling.} Proposition~\ref{prop:temporal} bounds the \emph{expected} spend under the Poisson update model. For an arbitrary arrival process, and as an \emph{unconditional} per-run guarantee, the re-release \emph{policy} additionally admits a hard ceiling when releases are budget-gated:
...
\end{prop}
\begin{proof}
Each release decrements the remaining budget by $\eps_q$ and is issued only when it is $\ge\eps_q$, so the remaining budget stays $\ge0$ and the cumulative spend is non-decreasing and bounded above by $B$. Once $\lfloor B/\eps_q\rfloor$ releases have been made the remaining budget is $<\eps_q$ and every further refresh is denied, giving the count.
\end{proof}

\noindent This is a worst-case guarantee (it holds for every arrival process, unlike the magnitude estimate) and it makes the refresh rate a formal \emph{optimization} in the remaining budget \emph{and} the volatility $\lambda$. Minimising the time-average staleness (proportional to $\tau$) subject to the re-release budget $\eps_q u_k(\lceil T/\tau\rceil+\lambda Tq)\le B$ (the $\lambda Tq$ term is the volatility spend of Proposition~\ref{prop:temporal}), the objective is monotone in $\tau$ and the optimum is on the budget boundary:
\[
\tau^\star(\lambda)=\frac{T}{\,B/(\eps_q u_k)-\lambda Tq\,},\qquad\text{valid when }\lambda Tq<\tfrac{B}{\eps_q u_k}.
\]
A shorter $\tau$ trips the gate (late ticks stale), a longer one wastes affordable freshness, and a higher update rate $\lambda$ \emph{raises} $\tau^\star$ since updates already consume part of $B$; at $\lambda{=}0$ it recovers $\tau^\star=T u_k\eps_q/B$, and once $\lambda Tq\ge B/(\eps_q u_k)$ the update load alone exhausts the budget (verified to spend exactly $B$ at the boundary, \texttt{experiments/temporal\_bound.py}). The accuracy theory of $\E[\eps_{\mathrm{temp}}]$ under a general renewal process remains future work; the ceiling and $\tau^\star(\lambda)$ are proven and unit-tested (\texttt{tests/test\_budget.py}).
...
\paragraph{Robustness of the forecast.} Five properties make the $\E[u_k]$ forecast dependable beyond the headline grid (\texttt{experiments/robustness.py}, \texttt{experiments/ood\_validation.py}). \emph{(i) Distribution-agnostic.} The closed form is exact for \emph{any} i.i.d.\ marginal, not only Zipf: on uniform, Zipf, and a lognormal-shaped marginal the $200$-trial empirical mean matches $\E[u_k]$ to $\le 0.2$ (a Monte-Carlo self-consistency check, not external validity---Section~\ref{sec:discussion}). \emph{(ii) Scale- and $\eps$-invariant.} It is unchanged by data volume (SF=1 vs SF=10, within $0.034$) and by $\eps_q$, since it depends only on the template distribution. \emph{(iii) Concentrated.} By Proposition~\ref{prop:concentration} the realized $u_k$ has standard deviation $\sqrt V=O(\sqrt m)$ (here $0.11$--$1.77$ at $m{=}20$, small relative to $\E[u_k]\approx15$--$20$), so a \emph{single} run lands within a fraction of a template of the forecast---the prediction is reliable per-workload, not merely in expectation. \emph{(iv) Conservative under correlation.} Real workloads are bursty, violating the i.i.d.\ assumption; we stress it with a sticky Markov process that repeats the previous template with probability $s$ (same stationary marginal). Burstiness clusters repeats and \emph{lowers} the distinct count, so the i.i.d.\ forecast \emph{over}-predicts $u_k$---by $1.09\times$ at $s{=}0.3$, $1.29\times$ at $s{=}0.6$, $2.72\times$ at $s{=}0.9$. Proposition~\ref{prop:sticky} gives this sticky-process $\E[u_k]$ in \emph{closed form}---a Markov generalisation of the occupancy formula, matching simulation to $<0.08$ (\texttt{experiments/markov\_occupancy.py})---so the burstiness gap is now exact, not merely bounded by $u_k\le m$. The error is in the \emph{safe} direction: positive correlation only makes $\eps_q\E[u_k]$ a looser \emph{upper} bound on the spend, so the privacy guarantee is never violated---only the savings are under-counted. The $B/m$ safe allocator is the worst-case form of this guarantee: since $u_k\le m$ for \emph{any} arrival process (i.i.d., bursty, or adversarial), $\eps_q=B/m$ never rejects. \emph{(v) Out-of-distribution.} To rule out that the accuracy is an artefact of drawing the workload from the same Zipf the closed form integrates, we run the deployable \emph{prefix}-forecaster on five families it is never told---two Zipf, plus uniform, lognormal-shaped, and a heavy Pareto tail. Forecasting the full $u_k$ from a $50\%$ prefix, the smoothed Good--Toulmin estimator~\cite{orlitsky2016unseen} stays within a mean $0.021$ (max $0.026$) in $u_k/k$ across \emph{all five}, Zipf and non-Zipf alike, cutting the naive prefix-distinct error by $76\%$; beyond horizon $t\approx1$ the unseen-species series degrades as expected, so we clamp the forecast to $[D_n,k]$, turning any divergence into the budget-safe over-forecast.

\begin{prop}[Sticky-Markov occupancy]\label{prop:sticky}
Under the sticky arrival process---$X_1\sim\{p_i\}$, and for $t\ge2$, $X_t{=}X_{t-1}$ with probability $s$ else a fresh draw $\sim\{p_i\}$---the expected distinct count is
$\E[u_k]=\sum_i[1-(1-p_i)(s+(1-s)(1-p_i))^{k-1}]$.
\end{prop}
\begin{proof}
The distinct templates visited equal the distinct values among the \emph{fresh} draws (a repeat re-emits the current template), and the number of fresh draws is $F=1+\mathrm{Binom}(k-1,1-s)$; conditioning on $F$ and using $\E[z^{\mathrm{Binom}(n,1-s)}]=(s+(1-s)z)^n$ with $z{=}1-p_i$ gives the result. At $s{=}0$ it recovers the i.i.d.\ $\E[u_k]$; at $s{=}1$ it is $1$. \end{proof}

\begin{prop}[General-Markov occupancy]\label{prop:markov}
For an arbitrary Markov arrival process with transition matrix $P$ and initial law $\nu$, let $Q_i$ be the sub-stochastic block of $P$ on the states $\neq i$ and $\nu_i$ the initial mass there. Then $\E[u_k]=\sum_i[1-\nu_i^{\!\top}Q_i^{\,k-1}\mathbf 1]$, of which the i.i.d.\ form and the sticky form (Proposition~\ref{prop:sticky}) are special cases.
\end{prop}
\begin{proof}
State $i$ is unvisited iff the chain stays off $i$ for all $k$ steps, with probability $\nu_i^{\!\top}Q_i^{\,k-1}\mathbf 1$; sum the complements. Verified against simulation and both special cases (\texttt{experiments/markov\_occupancy.py}). \end{proof}
...
\paragraph{Where it matters most: naive's failure regimes.} Two regimes turn the structural advantage into a stark, concrete gap over naive composition, measured on the real middleware (\texttt{experiments/killer\_cases.py}; Adult, $B{=}10$, $k{=}100$, $12$ trials, Table~\ref{tab:killer}). \emph{(A) Budget exhaustion.} A monitoring dashboard re-issues the same KPI tile at a fixed $\eps_q{=}1$: naive spends fresh budget each time and is \emph{exhausted after $B/\eps_q=10$ queries}, answering only $10/100$, while workload-aware caching answers all $100/100$ (the $90$ repeats are free)---a $10\times$ gap in queries served. \emph{(B) Accuracy at a fixed total budget.} To answer all $100$ queries, naive must spread the budget thinly ($\eps_q=B/k=0.1$), incurring per-query error ${\approx}10$; concentrating the same budget on the $u_k{=}1$ distinct template ($\eps_q=B/u_k$) makes the single release essentially exact (sub-rounding noise). \emph{(C, honest counter.)} On a genuine drill-down where every query is unique there is no structure to exploit: both answer all $100$ at the same error ($\approx 9.6$). We include C precisely because the model predicts the no-win case rather than overclaiming it.

\begin{table}[h]
\footnotesize\centering
\caption{Naive's failure regimes vs.\ workload-aware (real middleware, Adult, $B{=}10$, $k{=}100$, $12$ trials). Caching converts naive's two failure modes---budget exhaustion and budget-spreading---into answered queries and accuracy; on truly unique workloads we honestly match naive.}
\label{tab:killer}
...
Repetitive, fixed budget $B$ & $100/100$ & $10.0$ & $100/100$ & $\mathbf{\approx 0}$ \\
Drill-down (all unique) & $100/100$ & $9.6$ & $100/100$ & $9.6$ \\
\bottomrule
\end{tabular}
\end{table} At equal \emph{total} budget, the predictive allocator instead targets accuracy, cutting per-query MAE by up to $\sim$17\% on low-skew Zipf workloads (and up to $32\%$ in the tightest-budget regime $B=1$); this gain is significant only at low skew (see Section~\ref{sec:predictive}) and the regime-independent result is the safe $\eps_q=B/m$ allocator of Table~\ref{tab:allocation}.
...
The semantic L2 cache fails in both directions: it admits matches on queries with different true answers (false positives), and it misses matches on queries that are provably equivalent (false negatives). Tree-kernel and embedding-based semantic matching are not a free DP optimization. The right architecture is to pair similarity with symbolic equivalence checking---predicate normalization, range merging, alpha-conversion---so the system admits a semantic match only when the queries are provably equivalent, preserving the post-processing property. The sound core of this is now \emph{implemented} (\texttt{src/dpdb/normalize.py}): predicate normalization---integer-comparison tightening ($\texttt{age}\ge30\equiv\texttt{age}>29$), column orientation, commutative \textsc{and}/\textsc{or} sorting, single-column range-merging (provably-unsatisfiable conjunctions canonicalised to one \textsc{false}), and single-table alias $\alpha$-renaming---keyed into the cache so a match is admitted only on provable equivalence. End-to-end on the live middleware (\texttt{experiments/semantic\_normalize.py}) it serves \texttt{age>=30} and an \textsc{and}-reordered variant free as equivalents of \texttt{age>29} while refusing the non-equivalent \texttt{age>=31} ($3$ paid / $2$ free), and the golden rule---collide \emph{only} provably-equivalent queries---is brute-force-verified over $60{,}000{+}$ adversarial predicate pairs with \emph{zero} false collisions. Equivalences beyond decidable predicate normalisation are decided by an \emph{optional} Z3-backed SMT layer (\texttt{src/dpdb/smt\_equiv.py}): a hit is admitted only when the solver proves the WHERE predicates equivalent, catching integer arithmetic ($\texttt{age*2>=60}\equiv\texttt{age>=30}$), range disjunctions, and De~Morgan rewrites, brute-force-verified with zero false collisions (\texttt{tests/test\_smt\_equiv.py}); only general \emph{join}-level equivalences remain future work (\S\ref{sec:future-work}).
...
Mean absolute errors scale as $3/(2\eps)=1.5/\eps$, the expected absolute value of the difference of two independent $\Lap(1/\eps)$ variables (its standard deviation is the larger $2/\eps$). The empirical mean tracks the $1.5/\eps$ reference to within a few percent across three orders of magnitude in $\eps$. The table is indexed by the \emph{per-release} $\eps$, but the five-level drill-down is five releases: by sequential composition the analyst pays a \emph{cumulative} $5\eps$ in total, so a fixed total budget $B$ split across the levels gives per-release $\eps=B/5$ and correspondingly larger error---the Dinur--Nissim view ties reconstruction to the \emph{total} budget spent, not the per-query value.
...
The allocator's quality hinges on how well it predicts $u_k$ from the warmup prefix. The default plug-in occupancy estimate (§\ref{sec:predictive}) is the \emph{naive} unseen-species estimator: its support is only the observed templates, so it cannot count the unseen and systematically under-predicts. Table~\ref{tab:predictors} compares it against the horizon-aware alternatives (Section~\ref{sec:related}) on Zipf workloads ($m=50$ templates, $k=100$, $40$ trials), as a function of the extrapolation factor $t=(k-n)/n$ (so $t=1$ means the warmup is half the workload). In the realistic regime where the warmup is at least a third of the workload ($t\le 2$), the Good--Toulmin family roughly halves the prediction error and is near-unbiased, while the plug-in under-predicts by $18$--$43\%$; raw Good--Toulmin diverges past its $t\le 1$ validity limit, and the smoothed estimator~\cite{orlitsky2016unseen} tames it ($27\%$ vs $193\%$ at $t=2$). For very short warmups ($t=4$, warmup $<k/4$) no extrapolator is reliable---beyond the $t\propto\log n$ range---so a conservative bound is the safe choice. Chao1 is roughly flat and increasingly over-predicts, confirming that it answers total richness, not the horizon-$k$ count. The smoothed estimator improves the \emph{prediction} accuracy, so we make it a selectable mode of the live allocator (\texttt{estimator=\textquotesingle smoothed\_gt\textquotesingle} in \texttt{src/dpdb/predictive.py}); the estimators themselves live in \texttt{src/dpdb/predictors.py}. Its effect on the \emph{allocation}, however, is a trade-off rather than a free win, and we report it as such: on the Zipf allocation benchmark (Table~\ref{tab:allocation}) switching the allocator from plug-in to smoothed-GT lifts the answered fraction from $81\%$ to $92\%$---its over-prediction of $\hat U$ makes the allocator spend more conservatively, so fewer late queries are rejected---but at higher per-release noise. Better $u_k$ accuracy is thus not automatically better allocation: smoothed-GT is the right default when coverage matters, the plug-in when per-release noise does. We additionally implement a \emph{drift-aware} streaming learner (EWMA, \texttt{src/dpdb/workload\_learning.py}) that decays stale template mass and re-centres the $u_k$ forecast on the current regime; on a workload whose distribution shifts mid-stream it tracks the post-shift horizon $\sim$$15\times$ more accurately than a non-forgetting full-history plug-in (\texttt{experiments/workload\_learning.py})---a planning-layer estimator that leaves the privacy guarantee (the public cap $m$ and the ledger) untouched.
...
\textbf{Closed form, safe ($\eps_q{=}B/m$, ours)} & \textbf{2.0} & \textbf{100/100} & 7.9 \\
...
\paragraph{Where a learned allocator structurally fails---and where it does not.}
A sharper question than ``can we tie'' is whether there are workloads where a BGTplanner-style learned allocator is \emph{beaten}, not merely matched. We enumerated five candidate failure regimes, each tied to a specific published mechanism (the forced $(A{+}1)T_0$ initial stage, the smooth GP temporal kernel, the discrete $A$-level action grid, the always-on $p_t(a)=\frac1A+\eta(\cdot)$ sampling, and the observed-accuracy reward channel), and adversarially stress-tested every one against an \emph{honestly-tuned} baseline rather than a convenient strawman. Three did not survive, and we report that plainly (Table~\ref{tab:bgtfail}): the cold-start tax evaporates once the budget-level grid is centred on $m$ (a competent deployer's choice) rather than placed adversarially above it; the regime-shift collapse was an artefact of omitting BGTplanner's OMD constrainer, and a faithful long-horizon pacing term recovers coverage; and the coarse-grid penalty is retunable, since $u_k\le m$ and $m$ is public, so a grid spanning $[1,m]$ covers the entire realisable optimum. Two regimes do survive. The first is a \emph{domain mismatch}, and it is the deepest: BGTplanner's allocator is driven by an observed reward (validation accuracy between FL rounds), but in DP-SQL the per-query error is exactly what privacy hides, so the only observable surrogate---the granted-$\eps$ multiset and rejection count, the reward our own bandit uses---is structurally blind to \emph{which} template each $\eps$ backs. On a weighted dashboard (one headline KPI at weight $10$, three minor tiles at weight $1$; weights are public config, hence DP-valid), two allocations with a \emph{byte-identical} observable proxy (same sorted $\eps$ multiset $\{5.13,1.62,1.62,1.62\}$, zero rejections, $100\%$ answered) differ by $2.71\times$ in realised weighted error on the live middleware ($0.188$ vs.\ $0.509$, $400$ trials; \texttt{experiments/unobservable\_reward\_case.py}), so a proxy-maximising bandit is provably indifferent between the good and bad assignment while our closed form sets $\eps_i\propto\sqrt{w_i}$ from the public weights deterministically, at zero exploration. The reward FL observes for free would here cost budget to recover: estimating a single arm's per-template error to $\pm0.1$ takes ${\approx}146$ fresh releases, ${\approx}24\times$ the whole budget $B$. We bound this honestly: under \emph{flat} weights the granted-$\eps$ proxy is a faithful surrogate (error is monotone in $\eps$), the two assignments coincide, and our advantage there reduces to the exploration tax, \emph{not} the reward signal---so the claim is scoped to weighted or cache-structured workloads, and BGTplanner is not defective in its own federated setting. The second survivor is that exploration tax itself, now isolated: even post-convergence the $p_t(a)\ge\frac1A$ floor routes a fixed fraction of queries to off-optimal $\eps$, a $1.3$--$2.3\times$ steady-state per-query-noise gap over the zero-exploration $\eps_q=B/m$; it narrows toward a tie under a decaying $\eta_t\propto1/\sqrt t$ schedule and reverses into a \emph{loss} under genuine drift (where the GP temporal kernel's re-exploration is legitimate tracking and $B/m$ is stale). The two survivors are thus a fundamental domain mismatch and a bounded, regime-dependent tax---stated with the regimes in which we tie or lose, not buried.

\begin{table}[h]
\footnotesize
\centering
\caption{Five candidate regimes where a BGTplanner-\emph{style} learned allocator (adapted to DP-SQL) might be beaten, each adversarially tested against an honestly-tuned baseline. Three were rejected as strawman-dependent; we report that rather than claim them.}
\label{tab:bgtfail}
\begin{tabular}{l|c|p{0.46\columnwidth}}
\toprule
Regime (mechanism) & Verdict & Outcome \\
\midrule
Unobservable reward, weighted (observed-accuracy channel) & \textbf{holds} & identical proxy $\to 2.71\times$ realised loss; we read public $w_i$; self-probe tax ${\approx}24B$ \\
Steady-state exploration (eq.~12--13 floor) & \textbf{holds$^\dagger$} & $1.3$--$2.3\times$ per-query noise; $\dagger$ ties under $\eta_t{\propto}1/\sqrt t$, loses under drift \\
\midrule
Cold-start / short horizon ($(A{+}1)T_0$ stage) & rejected & grid centred on $m$ ties us ($0.98\times$) \\
Abrupt regime shift (smooth GP kernel) & rejected & faithful OMD pacing recovers coverage \\
Coarse action grid ($A{=}5$ levels) & rejected & retunable; span $[1,m]$ covers the optimum \\
\bottomrule
\end{tabular}
\end{table}
...
A natural objection is that our closed-form models were derived for single-table COUNT/SUM/AVG, where the sensitivity $\Delta f$ is a public constant ($1$ for COUNT, the column bound $B_c$ for SUM/AVG; Section~\ref{sec:threat}), and that they break for multi-table JOINs, whose sensitivity analysis is notoriously harder. We are precise about which part of the system transfers unchanged and which part genuinely degrades, and we \emph{measure} the transfer on a real TPC-H join (Figure~\ref{fig:joins}).

\begin{proposition}[Mechanism-agnostic forecast]
\label{prop:agnostic}
$\E[u_k]$, the savings $S(k)=1-\E[u_k]/k$, the workload-aware spend $\eps_q\E[u_k]$, and the relative utility ratio $k/\E[u_k]$ are functions of $(m,\{p_i\},k)$ alone; they are invariant to the per-query DP mechanism and to its sensitivity $\Delta f$, and therefore transfer from single-table aggregates to any query class---joins included---unchanged.
\end{proposition}
\noindent\emph{Proof.} Each is built from $\E[u_k]=\sum_i[1-(1-p_i)^k]$ (Prop.~\ref{prop:eu}), a function of the arrival distribution only; $\Delta f$ enters solely the noise scale $\Lap(\Delta f/\eps_q)$ and cancels in $k/\E[u_k]$. The single-table results are the $d_{\max}{=}1$ instance of the join-parameterized framework. \hfill$\square$

\paragraph{The occupancy/spend forecast is sensitivity-agnostic.}
Inspect what the forecasting layer consumes. The expected distinct count $\E[u_k]=\sum_i[1-(1-p_i)^k]$ (Prop.~\ref{prop:eu}), the savings ratio $S(k)=1-\E[u_k]/k$ (Prop.~\ref{prop:savings}), the three limits, the skew-monotonicity (Prop.~\ref{prop:zipf}), and the concentration bound $\Var[u_k]\le m/4$ (Prop.~\ref{prop:concentration}) are functions of the \emph{query stream} alone---the distribution $\{p_i\}$ over distinct query identities, the length $k$, and the \emph{count} $m$ of distinct releases. None references what a query computes or how sensitive it is, because occupancy---how many distinct items appear in $k$ i.i.d.\ draws---is a property of the arrival distribution, not of the data or the noise. Two consequences transfer to a JOIN workload \emph{unchanged}. (i)~The \emph{spend} forecast: under basic sequential composition the chargeable budget is $\eps_q\,\E[u_k]$, since an exact (cache-key) repeat---a JOIN repeat included---is served free as post-processing of an already-released DP answer. (ii)~The \emph{utility ratio} $k/\E[u_k]$ (Prop.~\ref{prop:utility}): the predicted error ratio is $(\Delta f/\eps_q^{\mathrm{naive}})/(\Delta f/\eps_q^{\mathrm{wa}})$, in which $\Delta f$ \emph{cancels}, so the relative-utility forecast is sensitivity-free and stays fully before-execution even when $\Delta f$ is unknown. What does \emph{not} transfer unchanged is the \emph{absolute} per-release accuracy verdict, which reads $\Delta f$ directly; we treat it below.

\paragraph{Completion: budget vs.\ answerability.}
The static completion guarantee splits into two statements with different status. \emph{Budget completion}---the ledger never overruns under the safe sizing $\eps_q=B/m$---holds for \emph{any} query class, because $u_k\le m$ is a purely combinatorial bound on the number of distinct query identities and $\eps_q\,u_k\le(B/m)m=B$ is pure bookkeeping. \emph{Answerability completion}---that each JOIN template actually receives a usable $\eps_q$-DP answer within its slice---does \emph{not} follow for free: a general equi-join has unbounded global sensitivity (below), so no finite-scale Laplace mechanism gives $\eps_q$-DP for that template at all, and producing an answer requires a finite per-template sensitivity bound (smooth/elastic/residual sensitivity or R2T truncation). The occupancy ledger is sensitivity-agnostic; answerability is not.

\paragraph{What changes is the per-release noise calibration.}
Moving to joins changes one input: the noise scale $\Lap(\Delta f/\eps_q)$. For a single-table COUNT, adding one row changes the answer by $1$; for a join COUNT (e.g.\ $R\bowtie S$ on a \emph{non-key} foreign-key/many-side attribute), adding one tuple to $R$ can match arbitrarily many tuples in $S$, so the worst-case \emph{global} sensitivity over all instances is \emph{unbounded} and the textbook Laplace calibration is undefined. The literature resolves this not by changing the budget accounting but by replacing the global constant with an instance-dependent or relaxed bound: smooth sensitivity, an instance-based smooth upper bound on local sensitivity~\cite{nissim2007smooth}; elastic sensitivity (the FLEX system), an efficiently computable upper bound on local sensitivity for equi-join counting queries, derived from per-attribute maximum-frequency statistics and deployed via its smooth version~\cite{johnson2018towards}; residual sensitivity, a polynomial-time smooth bound for multi-way join counts (without self-joins) that provably dominates elastic sensitivity~\cite{dong2021residual}; and R2T, which is a different mechanism---it truncates each entity's join contribution at a \emph{privately chosen} threshold $\tau$, making the truncated query's global sensitivity finite ($=\tau$), and is the first to handle foreign keys \emph{together with} self-joins for arbitrary SPJA queries~\cite{dong2022r2t}. These are not interchangeable drop-in values for $\Delta f$: the smooth-sensitivity route (smooth/elastic/residual) generally costs an $(\eps,\delta)$ relaxation rather than pure $\eps$-DP, and R2T's truncation spends extra budget to select $\tau$ and injects clipping \emph{bias}. The cache and the distinct-release count are untouched; the ledger semantics (pure $\eps$ vs.\ $(\eps,\delta)$, plus $\tau$-selection budget) are not.

\paragraph{Worked example (TPC-H).}
Consider \texttt{SELECT o\_orderpriority, COUNT(*) FROM orders JOIN lineitem ON l\_orderkey = o\_orderkey GROUP BY o\_orderpriority}, with the privacy unit an \emph{order} (entity/FK-DP: adding or removing an order also adds or removes its line items). Removing one order changes the count in its priority group by that order's number of line items, so the worst-case local sensitivity equals the maximum line-items-per-order; without an a-priori cap the global sensitivity is unbounded. If the schema declares a public foreign-key multiplicity bound $d_{\max}$ (TPC-H generates $1$--$7$ line items per order, so $d_{\max}=7$), then capping makes $\Delta f=d_{\max}$ a public \emph{global}-sensitivity constant---the conservative, $B_c$-style route, analogous to the column bound we already assume for SUM/AVG, and deliberately looser than the instance-optimal elastic/residual sensitivity. The join COUNT is then calibrated with $\Lap(d_{\max}/\eps_q)$. GROUP BY over a single base table costs $\eps_q$ by parallel composition (each base tuple, under the add/remove unit-tuple model, lies in one group), exactly as in the single-table treatment; over a join this is not automatic, because a base tuple's fan-out can span several output groups---parallel composition applies per group only after the join-sensitivity bound caps each base tuple's \emph{total} cross-group contribution (here, at $d_{\max}$). With that bound in place, the occupancy/spend forecast and the $B/m$ sizing apply unchanged.

\paragraph{What honestly degrades.}
We do not claim the join case is free. (1)~The \emph{absolute} per-release $(\alpha,\beta)$ accuracy verdict reads $\Delta f$ directly (the Laplace tail gives $\alpha=(\Delta f/\eps_q)\ln(1/\beta)$), so for joins it needs an a-priori \emph{public} sensitivity bound---a declared maximum join degree / foreign-key multiplicity, analogous to $B_c$. Absent such a declaration the equi-join's $\Delta f$ is data-dependent/unbounded and the \emph{absolute} verdict is no longer fully before-execution; the \emph{relative} utility ratio $k/\E[u_k]$ is unaffected, since $\Delta f$ cancels. (2)~With no public bound one falls back to data-dependent bounds (smooth/elastic/residual sensitivity, or R2T's privately chosen $\tau$): the accuracy forecast then loses its closed-form point value and must be reported as a data-dependent bound that typically requires an $(\eps,\delta)$ relaxation and, for truncation, carries a bias term---not merely an inflated Laplace variance. (3)~Self-joins are the hard regime: they correlate individuals' contributions so that adding one entity can raise the degrees of many others, which can break truncation itself---precisely the case R2T was built for (plain self-join-free multi-way joins are already handled by residual sensitivity). (4)~Net: the \emph{spend} forecast, the \emph{utility ratio}, and \emph{budget completion} survive intact, because they never touched $\Delta f$; the \emph{absolute} accuracy verdict and \emph{answerability} are what degrade, and only when no public degree bound is available.

\paragraph{Tractability verdict.}
The occupancy forecast stays $O(m)$ per evaluation of $\E[u_k]$ regardless of join complexity, because its cost is set by the number of distinct query identities, not by any one query's structure, and it needs only $\{p_i\}$ and $k$---no degree bound. What gets harder is the orthogonal per-query problem of bounding $\Delta f$ for a join, where global sensitivity is infinite but local sensitivity at a fixed instance is finite---exactly the gap the cited works close. Integrating a residual-sensitivity~\cite{dong2021residual} smooth bound (under an $(\eps,\delta)$ relaxation) or an R2T~\cite{dong2022r2t} private truncation, behind the same exact-repeat cache, leaves the spend forecast unchanged; the accuracy verdict, however, inherits the data-dependence, $(\eps,\delta)$ semantics, and (for R2T) truncation bias, and becomes a-priori public only under a declared degree/max-frequency bound.

\paragraph{Empirical confirmation (TPC-H).}
We run the worked example as a \emph{twin} experiment (\texttt{experiments/join\_validation.py}): one Zipf identity stream over the $m{=}5$ order-priority pool, instantiated both as the single-table COUNT ($\Delta f{=}1$) and as the \texttt{orders}$\bowtie$\texttt{lineitem} COUNT ($\Delta f{=}d_{\max}{=}7$) on the real $1.5$M-order / $6$M-line-item tables ($600$ trials, $\alpha\in\{0,\dots,2\}$, $k\in\{10,\dots,100\}$). The forecast is \emph{identical} on the two sides---realized distinct count, $\E[u_k]$, savings $S(k)$, and spend $\eps_q u_k$ coincide trial-for-trial, since occupancy reads only the identity stream---and tracks the closed form within $1.4\%$ on average ($17/20$ cells under $3\%$, the same accuracy as the single-table grid; Figure~\ref{fig:joins}, left). The \emph{only} quantity that moves is the per-release noise: the answered-release MAE is $7.6\times$ larger on the join side (Figure~\ref{fig:joins}, right), matching the $d_{\max}{=}7$ sensitivity ratio, while the relative utility ratio $k/\E[u_k]$ is unchanged because $\Delta f$ cancels. The sensitivity-agnostic transfer is thus not just argued but \emph{measured}: the forecast holds on a real multi-table join exactly as on the single table.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_joins}
\caption{Twin single-table vs.\ join experiment (TPC-H, $m{=}5$ priority pool, $d_{\max}{=}7$, $600$ trials). \emph{Left:} the realized distinct count for the single-table ($\circ$) and join ($\times$) workloads lands on the \emph{same} analytical $\E[u_k]$ curve. \emph{Right:} only the per-release noise scale moves, by $\Delta f$ ($1$ vs $d_{\max}{=}7$).}
\label{fig:joins}
\end{figure}
...
The Zipf parametrization in this paper is a controlled sweep, not a measurement of real analyst behaviour. We partially close this on Redbench~\cite{redbench2025}---$30$ production-\emph{derived} Amazon Redshift traces---where forecasting the full distinct-query count from only the first $50\%$ of each timeline via a smoothed Good--Toulmin unseen-species correction cuts the budget-prediction error by $45\%$ ($0.22\!\to\!0.12$ in $u_k/k$); running the model on \emph{raw} production SQL end-to-end through a DP engine (Redbench is trace-derived synthesis, not raw SQL) would close the remaining external-validity gap.
...
The semantic L2 cache of §\ref{sec:semantic} fails because tree-kernel and embedding similarity do not imply alpha-equivalence. The sound predicate-normalization core of the fix is now implemented (\texttt{src/dpdb/normalize.py}: integer tightening, column orientation, commutative \textsc{and}/\textsc{or} sorting), admitting a match only on provable equivalence and serving e.g.\ $\texttt{age}\ge30\equiv\texttt{age}>29$ free with no false matches. Extending it with range-merging and a full SMT equivalence check for general join/arithmetic rewrites is the remaining future work.
...
Section~\ref{sec:joins} analyzes this extension in full: the occupancy/spend forecast and the $B/m$ completion guarantee are sensitivity-agnostic and transfer to joins unchanged, while only the per-release noise calibration $\Delta f$ becomes hard (unbounded global sensitivity), to be supplied per template by smooth/elastic/residual sensitivity~\cite{nissim2007smooth,johnson2018towards,dong2021residual} or R2T truncation~\cite{dong2022r2t}. Implementing one such bound behind the exact-repeat cache, and measuring the accuracy-verdict degradation, is the most direct path to a thesis-grade extension. We take a first step: an elastic/smooth-sensitivity bound $\mathrm{ES}(0){=}\max(\mathrm{mf}_L,\mathrm{mf}_R)$ (FLEX-style~\cite{johnson2018towards}) is implemented and brute-force-verified to upper-bound the true local sensitivity over $50$k constructed join instances (\texttt{src/dpdb/join\_sensitivity.py}). Honestly, on TPC-H \texttt{orders}$\bowtie$\texttt{lineitem} it yields \emph{no} accuracy gain---the measured max-frequency already equals $d_{\max}{=}7$---so the instance-aware $(\eps,\delta)$ route helps only when the realized frequency sits well below a tight public clamp (e.g.\ \texttt{part}$\bowtie$\texttt{lineitem}); it is an opt-in capability that does not change the default pure-$\eps$ $d_{\max}$ mechanism.
...
\paragraph{Threats to validity.} I separate three. \emph{Construct.} The headline ``$\E[u_k]$ within $<3\%$ in 21/24 cells'' (Section~\ref{sec:eval}) is a Monte-Carlo \emph{self-consistency} check, not an external-validity test: the empirical $u_k$ is sampled from the very Zipf distribution the closed form integrates, so the $30$-trial mean converges to $\E[u_k]$ \emph{by construction}. It validates the estimator's arithmetic and convergence, not the claim that real workloads are i.i.d.\ Zipf. Per-release MAE excludes rejected queries, so we report the answered fraction in the same cells. \emph{Internal.} The budget forecast assumes a \emph{non-adaptive} analyst whose template choices are independent of the released noise. Privacy is unaffected---DP composition is adaptive-safe and total spend is hard-capped at $B$ by construction---but $\E[u_k]$ is an \emph{average-case} forecast: against a worst-case adaptive analyst the only valid bound is $u_k\le\min(k,m)$, i.e.\ $\eps_{\mathrm{wa}}\le\eps_q\,m$, which is exactly the $B/m$ safe allocator (Table~\ref{tab:allocation}). I now \emph{measure} this rather than only assume it: driving a genuinely noise-adaptive analyst---one that reads each released noisy count and chooses its next query from it---through the \emph{live} middleware (\texttt{experiments/adaptive\_analyst.py}), and stressing sticky/anti-sticky Markov arrivals (\texttt{experiments/arrival\_processes.py}), the realized $u_k\le m$ in every policy and trial, so the $B/m$ allocator spends $\le B$ and never rejects even against an adversary that couples its queries to the DP noise; the forecast is exact for the non-adaptive baseline ($u_k{=}19.9$ vs.\ $\E[u_k]{=}19.6$) and only the average-case estimate---never the privacy bound---degrades under adaptivity. Under the worst-case ``hunting'' analyst that steers each query by the largest released noisy count, $u_k$ stays $\le m$ in all $40$ seeded trials while the forecast becomes optimistic by only $+12.5\%$ ($u_k{=}22.0{\pm}0.23$ vs.\ $19.6$; \texttt{experiments/adaptive\_hunting.py}). The constructed mixed-workload agreement is a ledger identity ($u_k$ is fixed by construction, so $\eps_q u_k$ is definitional with zero trial variance), not an independent prediction. \emph{External.} The synthetic workloads are Zipf sweeps plus TPC-H/Adult parametric families; we add an external test on Redbench~\cite{redbench2025} ($30$ production-\emph{derived} Amazon Redshift traces), where the prefix forecast cuts budget-prediction error by $45\%$ ($0.22\!\to\!0.12$). The residual gap is honest: Redbench is trace-derived synthesis, not raw production SQL end-to-end through a DP engine---the single largest remaining external-validity threat. The forecast is moreover \emph{composition-agnostic} (\texttt{experiments/composition\_invariance.py}): it predicts the paid-release count $u_k$, the lever every composition theorem prices, so the savings equal $S(k)$ exactly under linear accounting (pure $\eps$-DP, zCDP-$\rho$) and stay $47$--$78\%$ under sub-linear advanced $(\eps,\delta)$ composition---the savings are not an artefact of basic composition. Scope is single-table aggregates; the multi-table join extension is analyzed \emph{and now executed live in the middleware} ($\Delta f{=}d_{\max}$, \texttt{experiments/live\_join.py}; Section~\ref{sec:joins}), while instance-optimal join sensitivity and renewal-process arrival accuracy remain out of scope (Section~\ref{sec:future-work}).

\paragraph{Reproducibility.} The middleware, all experiment generators, and the analysis scripts are in the repository, and the canonical result CSVs the tables cite are committed under \texttt{results/}. The model-validation, predictor, and allocation experiments use fixed integer seeds and are bit-reproducible (the analysis of the predictive table is regenerated by \texttt{experiments/analyze\_predictive.py}); the large six-sweep campaign is \emph{distributionally} reproducible---re-running its generators yields statistically equivalent numbers, and the committed CSVs from the submitted runs are the canonical artifact.
...
X. Zhang, Y. Zhou, M. Hu, D. Wu, P. Liao, M. Guizani, and Q. Z. Sheng. 2025.
...
\newblock {\em IEEE Transactions on Services Computing} 18, 6 (2025), 3523--3531. DOI:10.1109/TSC.2025.3616355. Also arXiv:2412.02934.
...
\bibitem{redbench2025}
S. Krid, M. Stoian, and A. Kipf. 2025.
\newblock Redbench: A Benchmark Reflecting Real Workloads.
\newblock In {\em aiDM @ SIGMOD}. arXiv:2506.12488. (Workloads synthesized from Amazon Redshift's Redset query traces.)
