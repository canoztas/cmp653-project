...
Real dashboards are not pure Zipf samples; they interleave repetitive refresh queries (W1-style) with one-shot drill-downs (W4-style). To test the model on heterogeneity directly, we constructed a mixed workload of $k=100$ queries with a controlled fraction $f$ of repetitive entries, so the true unique count is $u_k = 1 + (1-f)k$ by construction. Table~\ref{tab:mixed} compares the empirical workload-aware budget to the model prediction $\eps_q \cdot u_k$ at $\eps_q = 0.5$.
...
The two new mechanisms compose. Combining the predictive allocator with a finite staleness tolerance $\tau$ (Section~\ref{sec:temporal}) yields lower MAE than temporal alone at every $\tau$ we tested (Table~\ref{tab:predictive-temporal}), in exchange for spending the full budget instead of the temporal-alone fraction. The choice between them is a deployment preference: \emph{cheap re-noising with higher noise} (temporal alone) versus \emph{full-budget spend with the lowest achievable noise} (combined).
