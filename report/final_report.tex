...
The middleware is a thin proxy between the analyst and a backend database (DuckDB by default, with an alternative PostgreSQL adapter behind an identical interface). Algorithm~\ref{alg:full} gives the full processing pipeline.
...
\caption{Cross-scale validation: same model, two TPC-H scales (SF=1 with 6M and SF=10 with 60M lineitem rows). Empirical means coincide within $0.034$ units.}
\label{fig:sf10}
\end{figure}
...
\caption{Per-pair-difference reconstruction error on the five-level drill-down, against the theoretical $1.5/\eps$ scaling (the expected absolute difference of two independent $\Lap(1/\eps)$ variables). Empirical mean and $p95$ both follow the predicted slope across three orders of magnitude in $\eps$.}
...
PINQ~\cite{mcsherry2009privacy}, PrivateSQL~\cite{kotsogiannis2019privatesql}, Chorus~\cite{johnson2020chorus}, and DOP-SQL~\cite{yu2024dopsql} all expose a fixed per-query $\eps_q$ that the analyst chooses up front. None of them set $\eps_q$ from a forecast of \emph{future workload-aggregate} consumption. Two systems do adapt budget in adjacent senses: APEx~\cite{ge2019apex} translates a per-query accuracy bound into the minimum-privacy $\eps$ for the \emph{current} query at runtime, and PrivateSQL~\cite{kotsogiannis2019privatesql} allocates budget across views \emph{offline} over a representative workload. The distinction here is that the predictive allocator chooses $\eps_q$ from an analytical forecast of future unique-template consumption (Proposition~\ref{prop:eu}), rather than from the current query's accuracy target or a static per-view split. Its mathematical justification rests directly on Proposition~\ref{prop:savings}: the optimal offline allocation is $\eps_q^* = B/u_k$, and the predictive allocator approximates $u_k$ online.
...
\paragraph{Interpretation.}
The predictive allocator delivers a modest MAE reduction (5--17\% lower than naive: 16.8\% at $\alpha=0$ down to 5.4\% at $\alpha=2$) at the cost of denying some queries when the budget runs out. The trade-off surface is visible: $\eps_q$ per release jumps from $0.1$ to $\approx 1.43 = B/m$, matching the theoretical optimum that the model derives from the predicted $\hat U \approx m$. Cache hits inherit whichever noisy answer was released at the time, so the cache becomes "locked in" to the early-release quality. To realize the full $B/U$ utility benefit, the mechanism must \emph{re-release} cached answers when the budget shows surplus---an extension I discuss in §\ref{sec:future-work}.

\paragraph{The gap across budget regimes.}
The MAE improvement at $B=10$ is not a one-off. Sweeping the total budget over $B\in\{1,5,10,50\}$ at $\alpha=1$, $k=100$ (Table~\ref{tab:budget-sweep}) shows MAE reductions of $32\%$ ($B{=}1$), $\sim$$0.5\%$ ($B{=}5$), $18\%$ ($B{=}10$), and $21\%$ ($B{=}50$) relative to naive. The reduction is non-uniform: it is largest in the tightest-budget regime ($B{=}1$, where every release matters) and is negligible at $B{=}5$ (consistent with the near-identical naive/predictive entries in Table~\ref{tab:budget-sweep}).
...
\begin{enumerate}
    ...
    \item \textbf{The toy $1-1/k$ rule is a corner case.} The milestone identified $1-1/k$ for perfect repetition; the model places this at Limit~A ($\alpha\to\infty$) and predicts the full interior between it and naive composition (Limit~B). At $\alpha=2$, $k=200$, $m=7$ the savings ratio is empirically $96.5\%$ ($\E[u_k]=6.93$, $S=1-6.93/200$) and the model predicts $96.5\%$ ($\E[u_k]=6.90$).
    \item \textbf{The cumulative-$\eps$ bound is loose for realistic multi-query MIA.} Shadow-model MIA AUC across W1--W4 ranges from $0.14$ to $0.58$, well below the cumulative bound $e^{\sum\eps_i}/(1+e^{\sum\eps_i})\to 1.0$ that current accounting assumes. Practically: privacy budget is overspent if the threat model assumes worst-case observation.
