...
Exact-repeat caching is a mechanism, not the central contribution. Returning a cached noisy result is a direct application of the post-processing property of DP and does not by itself answer \emph{when and by how much} caching helps. We promote the contribution to a model that gives quantitative predictions for arbitrary workload distributions and recovers caching as its $\alpha\to\infty$ corner case.
...
\begin{itemize}
    ...
    \item AVG($c$): we treat AVG as a conservative single-mechanism release with $\Delta f = B_c$, the worst-case bound corresponding to a group of size one. This avoids the implicit $n$-leak of the empirical-mean mechanism $\Lap(B_c/(n\eps_q))$, since $n$ itself is private. A tighter SUM+COUNT decomposition with explicit $2\eps_q$ accounting per group would lower the noise variance on AVG releases for large groups but is not yet implemented in the middleware; we list it as future work in Section~\ref{sec:future-work}.
\end{itemize}
...
\paragraph{Setup.} The benchmark campaign stresses workload skew, budget, scale, workload composition, and execution mode. We run \textbf{six experimental sweeps} for a total of \textbf{4{,}155 trials} ($\sim 150{,}000$ individual queries):
...
\begin{itemize}
    ...
    \]
    where $C$ is defined recursively on aligned subtrees (Section 2 of \cite{collins2001convolution}). We normalize as
    \[
    K_{\mathrm{norm}}(T_1, T_2) = \frac{K(T_1,T_2)}{\sqrt{K(T_1,T_1)\,K(T_2,T_2)}} \in [0,1].
    \]
...
Returning a cached noisy answer for a query that is \emph{not} alpha-equivalent to the original falls outside the post-processing guarantee: the cached answer was tuned to a different question. The semantic cache is therefore \emph{not} privacy-free unless the matched queries are equivalent. We treat it as a measurable approximation layer: it trades exact-utility for additional budget savings, and the paper reports both sides of the tradeoff.

\paragraph{Empirical results.}
We compare three modes on Zipf parametric workloads ($k=50$, 30 trials per cell):
...
\paragraph{What we found.} The campaign produced four findings worth stating explicitly.
