...
Reading Table~\ref{tab:related-comparison} column by column, the combination \textbf{A+B+D}---closed-form workload model + predictive online allocator + temporal coupling---is empty in the surveyed literature. Workload-aware DP mechanisms (Matrix Mechanism~\cite{li2010mm}, HDMM~\cite{mckenna2018hdmm}, PrivateSQL~\cite{kotsogiannis2019privatesql}) model linear-query correlation or representative-workload views, not template repetition; their allocation is offline. Online / adaptive DP (MWEM~\cite{hardtligett2012mwem}, PMW~\cite{hardtrothblum2010pmw}, median mechanism~\cite{rothroughgarden2010median}, APEx~\cite{ge2019apex}, Pythia~\cite{kotsogiannis2017pythia}) adapts queries, accuracy, or algorithms at runtime but does not forecast future budget consumption from an explicit workload distribution model, and none addresses temporal freshness. Production DP-SQL (Tumult Analytics~\cite{berghel2022tumult}, OpenDP/SmartNoise~\cite{opendp2023}, Bounded User Contribution~\cite{wilson2020bounded}) provides correct accounting and scale but neither claims nor implements the analytical model layer. Finally, the AI/AST tools we use (tree kernels, CodeBERT, GraphCodeBERT) come from the representation-learning literature and have not been applied to DP-SQL cache lookup; our contribution there is the bridge---and the \emph{honest negative} that semantic similarity alone is not a substitute for formal equivalence (§\ref{sec:semantic}).
