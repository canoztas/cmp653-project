...
\paragraph{Robustness of the forecast.} Four properties make the $\E[u_k]$ forecast dependable beyond the headline grid (\texttt{experiments/robustness.py}). \emph{(i) Distribution-agnostic.} The closed form is exact for \emph{any} i.i.d.\ marginal, not only Zipf: on uniform, Zipf, and a lognormal-shaped marginal the $200$-trial empirical mean matches $\E[u_k]$ to $\le 0.2$ (a Monte-Carlo self-consistency check, not external validity---Section~\ref{sec:discussion}). \emph{(ii) Scale- and $\eps$-invariant.} It is unchanged by data volume (SF=1 vs SF=10, within $0.034$) and by $\eps_q$, since it depends only on the template distribution. \emph{(iii) Concentrated.} By Proposition~\ref{prop:concentration} the realized $u_k$ has standard deviation $\sqrt V=O(\sqrt m)$ (here $0.34$--$1.22$ at $m{=}20$), so a \emph{single} run lands within a fraction of a template of the forecast---the prediction is reliable per-workload, not merely in expectation. \emph{(iv) Conservative under correlation.} Real workloads are bursty, violating the i.i.d.\ assumption; we stress it with a sticky Markov process that repeats the previous template with probability $s$ (same stationary marginal). Burstiness clusters repeats and \emph{lowers} the distinct count, so the i.i.d.\ forecast \emph{over}-predicts $u_k$---by $1.09\times$ at $s{=}0.3$, $1.29\times$ at $s{=}0.6$, $2.72\times$ at $s{=}0.9$. The error is in the \emph{safe} direction: positive correlation only makes $\eps_q\E[u_k]$ a looser \emph{upper} bound on the spend, so the privacy guarantee is never violated---only the savings are under-counted. The $B/m$ safe allocator is the worst-case form of this guarantee: since $u_k\le m$ for \emph{any} arrival process (i.i.d., bursty, or adversarial), $\eps_q=B/m$ never rejects.
