...
\begin{enumerate}
    ...
    \item \emph{``Leakage and savings models are very limited.''} We run three leakage experiments---single-query MIA, drill-down differencing, and shadow-model MIA---against tight theoretical bounds (Section~\ref{sec:leakage}), backed by a $4{,}155$-trial validation campaign (Section~\ref{sec:eval}).
\end{enumerate}
...
The naive-DP baseline is the standard per-query composition used by prior fixed-$\eps$ DP-SQL systems: every release spends fresh budget, so answering $k=100$ queries at $\eps_q=1$ costs the full $\eps=100$ on every workload (the NAIVE rows above). Workload-aware accounting answers the \emph{same} 100 queries at identical or better MAE while spending only $\eps_q\cdot u_k$: $1.0$ on the repetitive W1 ($100\times$ less budget), and $3.0$/$5.0$/$7.0$ on the W2 parametric and W3 uniform pools ($33\times$, $20\times$, $14.3\times$ less), driven by $93$--$99\%$ cache-hit rates. On the genuine drill-down W4, where every query is unique, it correctly spends the same $\eps=100$ as naive ($1\times$, no win)---the model predicts this $S(k)=0$ case exactly rather than overclaiming. At equal \emph{total} budget, the predictive allocator instead wins on accuracy, cutting per-query MAE by $5$--$17\%$ on Zipf workloads (and up to $32\%$ in the tightest-budget regime $B=1$), at the cost of rejecting $12$--$31\%$ of queries once the budget is exhausted. In short: workload-aware accounting wins on \emph{budget} (up to $100\times$, Figure~\ref{fig:savings}) on repetitive and parametric workloads, the predictive allocator wins on \emph{accuracy} ($5$--$17\%$) at fixed budget, and neither overclaims on truly unique drill-downs.
...
The estimator above answers \emph{how to predict} $u_k$; a separate question is \emph{which policy} should spend the prediction. BGTplanner~\cite{zhang2024bgtplanner} (Section~\ref{sec:related}) allocates budget online with a contextual bandit driven by \emph{observed utility}---validation accuracy in federated learning. In a DP-SQL setting the per-query error is not observable (the true answer is exactly what privacy hides), so a utility-driven bandit cannot see its reward. We therefore compare three online policies that use only \emph{observable} signals---budget spent and query rejections---end to end on Zipf workloads ($m=20$, $k=100$, $B=10$, $60$ trials): naive ($\eps_q=B/k$), our closed form ($\eps_q=B/\hat U$, with $\hat U$ from the plug-in or the smoothed estimator), and an $\eps$-greedy bandit that explores denominators $d\in\{k,k/2,k/4,m\}$ with $\eps_q=B/d$ (reward $=\eps_q$ on a granted release, a penalty on a budget-exhausted rejection). Table~\ref{tab:allocation} reports two error figures: end-to-end MAE over all answered queries, and a \emph{fresh-release} MAE that excludes cache hits inheriting an early answer, isolating allocation quality. The harness is \texttt{experiments/allocation\_policy\_comparison.py}.

Three things stand out, and we report them plainly. First, every budget-aware policy roughly halves the fresh-release noise of naive ($4.8$/$3.9$ vs.\ $9.9$): naive divides by $k=100$ but only $\sim u_k\approx 16$ distinct queries occur, so it spends just $1.6$ of $B=10$ and leaves $84\%$ of the budget unused---this is the robust, expected win for any budget-aware allocator. Second, a simple bandit is competitive with---here, slightly better than---our closed-form allocator, for reasons already on our roadmap: the closed form's conservative low-$\eps$ warmup releases are cached and then repeated (the cache-re-release limitation of Section~\ref{sec:future-work}), and its $\eps_q=B/\hat U$ spends the entire budget, rejecting $18\%$ of late queries; the bandit's exploration incurs neither cost. Third, swapping the plug-in for the smoothed estimator answers \emph{more} queries ($92$ vs.\ $82$) but at higher noise, because that estimator \emph{over}-predicts $\hat U$ and so under-spends per release---the predictor and the policy interact, and the best denominator is not simply the most accurate $\hat U$. The constructive reading is that the model and a learned policy are complementary: a model-driven bandit that takes the closed-form $\hat U$ as its context (a BGTplanner-style policy seeded with our forecast) is the natural synthesis, pairing a principled target with online adaptivity. The model remains the contribution; the allocator is one way to spend it.

\begin{table}[h]
\footnotesize
\centering
\caption{Online allocation policies, end to end (Zipf, $m=20$, $k=100$, $B=10$, averaged over $\alpha\in\{0.5,1,1.5,2\}$, $60$ trials). \emph{MAE} is over all answered queries; \emph{fresh} excludes cache hits that inherit an early answer. Budget-aware policies halve fresh-release noise vs.\ naive, which leaves $84\%$ of the budget unused; a simple bandit is competitive with the closed form.}
\label{tab:allocation}
\begin{tabular}{l|cccc}
\toprule
Policy & MAE & fresh & answered & $\eps$ used \\
\midrule
Naive ($\eps_q{=}B/k$) & 10.1 & 9.9 & 100/100 & 1.6 \\
Closed form, plug-in (ours) & 7.3 & 4.8 & 82/100 & 10.0 \\
Closed form, smoothed-GT & 10.6 & 9.6 & 92/100 & 9.0 \\
Bandit explorer & \textbf{4.3} & \textbf{3.9} & 100/100 & 5.9 \\
\bottomrule
\end{tabular}
\end{table}
