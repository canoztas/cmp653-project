...
\paragraph{Two cases our forecast covers that reactive systems do not natively expose---and where we are merely on par.} We frame the comparison not as ``beating'' the baselines but as concrete situations a custodian faces, and we are honest that on the raw numbers we are level, not ahead. \emph{Case 1 (sizing \emph{before} spending).} A team must fix a monthly per-query $\eps_q$ for a privacy-protected dashboard before the month starts. At $t{=}0$ our model answers two \emph{separate} questions: \emph{will the workload complete within $B$?}---a hard yes under the $B/m$ sizing, since $u_k\le m$---and \emph{will each release meet accuracy $(\alpha,\beta)$?}---a \emph{per-release} probability, reported individually rather than as a joint guarantee (Appendix~\ref{app:apriori}). Reactive DP caches (Turbo, CacheDP) and accuracy-aware explorers (APEx) do not natively expose either before execution: they discover the answer only by spending budget query-by-query, and remaining-budget estimators are undefined at zero history. \emph{Case 2 (capacity planning from a prefix).} From only the first half of a real workload, the unseen-species forecast predicts the full distinct-query count---and hence the total spend---halving the plug-in's error on production-derived traces (Section~\ref{sec:redbench}); a reactive system has no such forward estimate. \emph{Where we do not lead, we are level.} Taking the baselines' \emph{own} published figures rather than re-running them, Turbo reports $1.7$--$15.9\times$ budget conservation~\cite{kostopoulou2023turbo} and CacheDP a reduction under an accuracy contract~\cite{mazmudar2023cachedp}; our exact-repeat caching realizes $14$--$33\times$ on the parametric pools and up to $100\times$ on pure repetition---the same order of magnitude. We present this as a \emph{ballpark placement from their reported numbers}, not a controlled head-to-head, since the workloads and generalization mechanisms differ (Turbo generalizes across overlapping linear queries; we reuse only exact repeats), so we claim \emph{parity}, not superiority. Likewise at a fixed budget the safe $\eps_q{=}B/m$ allocator simply \emph{reaches} a tuned $\eps$-greedy bandit's best operating point without paying its exploration tax (MAE $2.0$ vs.\ $3.8$ for the \emph{same} answer rate, because the bandit must explore to find the point we compute in closed form; Appendix~\ref{app:allocator}). The contribution is therefore the \emph{before-execution} deliverable that the others lack, not a larger savings multiplier.
...
In DP-SQL the per-query error $|\text{noisy}-\text{true}|$ is exactly what privacy hides, so a reward-driven allocator cannot observe its true objective. Consider the weakest---but common---surrogate: an \emph{assignment-free} reward that sees only the sorted granted-$\eps$ multiset and the rejection count, blind to which template each $\eps$ backs. On a weighted dashboard (one KPI at weight $10$, three tiles at weight $1$), two allocations with an \emph{identical} assignment-free proxy (same sorted $\eps$ multiset, zero rejections, $100\%$ answered) differ by $2.71\times$ in realized weighted error on the live middleware ($0.188$ vs.\ $0.509$, $400$ trials), so a learner \emph{restricted to that proxy} cannot prefer the good assignment. We deliberately do \emph{not} overclaim this as ``allocation cannot be learned'': a genuine \emph{contextual} bandit that observes the query identity and the public weights is \emph{not} blind---it could read the weights and tilt $\eps$ accordingly, exactly as our closed form does ($\eps_i\propto\sqrt{w_i}$). The honest point is narrower: the information that closes the gap is the \emph{public weights}, so a learner must be \emph{given} the same public structure as context, at which point it is computing the allocation rather than discovering it from a reward; and under \emph{flat} weights even the assignment-free proxy is faithful and the gap vanishes.
