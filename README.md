...
![tests](https://img.shields.io/badge/tests-233%20passing-brightgreen) ![python](https://img.shields.io/badge/python-3.10%2B-blue) ![dp](https://img.shields.io/badge/privacy-differential-purple)
...
Every number, table, and figure in the paper is regenerated by a script in `experiments/`, with fixed integer seeds. Full pipeline from a fresh checkout:
...
**4. Regenerate the paper's results** (script → paper mapping):
```bash
python experiments/model_validation.py --trials 30   # Table 2 main grid + limits figure
python experiments/redbench_validation.py            # Redbench findings incl. the 45% headline
python experiments/ood_validation.py                 # out-of-distribution check (finding 5)
python experiments/join_validation.py                # twin single-table vs join (Fig. joins)
...
python experiments/workload_learning.py              # drift-aware forecasting (~15x)
...
```
Outputs land in `results/` as CSVs; the model-validation and allocation experiments are deterministic within a fixed environment (seeds are hard-coded), and the large sweeps are distributionally reproducible.

```bash
python demo/app.py                 # interactive web demo (EN/TR), runs the real pipeline
```
...
```
src/dpdb/        the middleware + the model        (model, predictive, budget, middleware, parser, normalize, smt_equiv, join_sensitivity, …)
experiments/     every figure/number has a script  (model_validation, redbench_validation, join_validation, …)
...
The full write-up is **[`report/course_paper.pdf`](report/course_paper.pdf)** — *Forecasting Privacy-Budget Consumption in Repeated DP-SQL Workloads*: eleven propositions (occupancy, savings, concentration, Markov/latent-state arrivals, temporal bounds, mechanism-agnostic joins), a 4,155-trial validation, the Redbench real-trace result, two leakage experiments, and honest limitations.
